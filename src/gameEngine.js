...
export function GameEngine(command, location, items, moveLocation, player, damagePlayer) {
    let response = [];
    ...
    if (words.length === 1) {
        ...
        response = parseTwoWordCommands(words, response, location, player, items, moveLocation, damagePlayer);
