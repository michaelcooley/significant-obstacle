...
export function GameEngine(command, location, items, moveLocation, player, damagePlayer, npcData, currentNPCLocation) {
    let response = [];
    command = command.toLowerCase();
    let words = command.split(' ');

    if (words.length === 1) {
        response = parseOneWordCommands(words[0], words, response, location, player, items, moveLocation, damagePlayer, currentNPCLocation, npcData);
